...
class WebServiceConnection extends PixzelleWebServiceConnection<Object, Void> {
    ...
    public ArrayList<Pair<String, String>> buildHeader() {
        ...
        try {
            headers.add(new Pair<>("X-User-Agent", "{\"agent\":\""
                    + getApplicationName(this.getContext())
                    + " Android" + "/"
                    + this.getContext().getPackageManager().getPackageInfo(this.getContext().getPackageName(), 0).versionName
                    + "\"}"));
        } catch (Exception e) {
            // TODO: 7/12/18 Report the error to new relic
            Logger.logError(e);
        }
        ...
        if (!Global.getStringKey(
                ...
                this.getWebService() != Definitions.CARD_TYPE &&
                this.getWebService() != Definitions.PAYMENT_CARD &&
        ...
        } else if (this.getWebService() == Definitions.PAYMENT_CARD) {

            if (Definitions.getCredentials() == null || Definitions.getCredentials().isEmpty()) {

                Logger.logDebug("token", Global.getStringKey(this.getContext(), Definitions.KEY_REGISTER_TOKEN()));
                headers.add(new Pair<>(Definitions.AUTH_TOKEN(), String.format(Definitions.HEADER_CONTENT(), Definitions.BEARER(), Global.getStringKey(this.getContext(), Definitions.KEY_REGISTER_TOKEN()))));

            } else {
                headers.add(new Pair<>("Authorization", Definitions.getCredentials()));
            }
